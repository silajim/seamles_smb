...
OtherLogger::OtherLogger(QString logpath, bool debug) : DbgPrint(false, debug), m_logpath(logpath)
{
    qDebug() << Q_FUNC_INFO;
}
...
void OtherLogger::print(LPCWSTR format, ...)
{
    ...
    if (!f)
    {
        QString name = m_logpath + "_SECURITY_OTHER_" + QDateTime::currentDateTime().toString("dd.MM-hh.mm.ss") + "_Start.log";
        f = new QFile(name);
